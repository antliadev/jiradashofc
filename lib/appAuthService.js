...
import { normalizeAccessProfile } from '../shared/access-rbac.js';
...
function publicUserFromSupabase(user, profile = null, permissions = []) {
  const role = normalizeAccessProfile(profile?.role || profile?.primary_role || 'dev_qa');
  return {
    id: user.id,
...
async function roleCodeForStorage(code) {
  const roleCode = normalizeAccessProfile(code);
...
async function replaceGrantedRoleAndPermissions(userId, grant) {
  const role = normalizeAccessProfile(grant.primary_role || 'dev_qa');
  ...
  // RBAC novo: permissões efetivas vêm do perfil em role_permissions.
  // Permissões individuais antigas são removidas acima e não são recriadas.
}

async function upsertProfileFromGrant(user, grant) {
  if (!supabase || !grant) return null;
  const googleName = String(user.user_metadata?.name || user.user_metadata?.full_name || '').trim();
  const displayName = googleName || String(grant.display_name || '').trim();
  const role = normalizeAccessProfile(grant.primary_role || 'dev_qa');
...
async function loadGrantedSessionUser(user) {
  ...
  if (authConfig.requireEmailConfirmation && !user.email_confirmed_at) {
    const authError = new Error('Confirme o email antes de acessar o Antlia Deliverable System.');
  ...
  if (!profile) {
    ...
    if (googleName && (!currentName || currentName === profile.email)) {
      const { error: updateNameError } = await supabase
        .from('profiles')
        .update({ display_name: googleName, last_login_at: new Date().toISOString() })
        .eq('user_id', user.id);
      if (!updateNameError) profile = { ...profile, display_name: googleName };
    }
  }
