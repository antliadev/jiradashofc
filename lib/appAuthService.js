...
function publicUserFromSupabase(user, profile = null, permissions = []) {
  const role = normalizeAccessProfile(profile?.role || profile?.primary_role || 'desenvolvedor_ba');
  return {
    id: user.id,
...
async function replaceGrantedRoleAndPermissions(userId, grant) {
  const role = normalizeAccessProfile(grant.primary_role || 'desenvolvedor_ba');
