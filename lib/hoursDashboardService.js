...
const CRAWFORD_CAPACITY_BY_COMPETENCE_SECONDS = Object.freeze({
  '2026-08': 100 * 60 * 60,
  '2026-09': 200 * 60 * 60
});
const CRAWFORD_USAGE_WINDOW_START_BY_COMPETENCE = Object.freeze({
  '2026-09': '2026-08'
});
...
function capacitySecondsForCompetence(projectConfig, competence) {
  if (projectConfig.key === 'CRAWFORD') {
    return CRAWFORD_CAPACITY_BY_COMPETENCE_SECONDS[competence] || projectConfig.capacitySeconds;
  }
  return projectConfig.capacitySeconds;
}
...
function accountableSecondsForCompetence(projectConfig, monthlyMap, competence) {
  ...
  const windowStart = projectConfig.key === 'CRAWFORD'
    ? CRAWFORD_USAGE_WINDOW_START_BY_COMPETENCE[competence]
    : null;
  if (windowStart) {
    return [...monthlyMap.entries()].reduce((total, [month, seconds]) => {
      return month >= windowStart && month <= competence ? total + seconds : total;
    }, 0);
  }
...
export function buildProjectHoursDashboard(worklogs, issues, selectedCompetence = null, projectKey = 'CRAWFORD') {
  ...
  if (projectConfig.key === 'CRAWFORD') {
    Object.keys(CRAWFORD_CAPACITY_BY_COMPETENCE_SECONDS).forEach(month => competenceCandidates.add(month));
  }
  ...
  const capacitySeconds = capacitySecondsForCompetence(projectConfig, competence);
  const state = capacityStatus(usedSeconds, capacitySeconds);
  const displayedUsedSeconds = projectConfig.billingMode === 'cumulative' ? usedSeconds : selectedPeriodSeconds;
  ...
  const monthlyConsumption = availableCompetences.map(month => {
    ...
    const monthCapacitySeconds = capacitySecondsForCompetence(projectConfig, month);
