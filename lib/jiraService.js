...
const dashboardCache = new NodeCache({ stdTTL: 180, checkperiod: 60, useClones: false });
...
function getConfiguredFieldValue(fields, envName, fallbackKeys = []) {
  const keys = [...parseEnvList(envName), ...fallbackKeys];
...
export function resolveBlockedFields(fields = {}) {
  return {
    blocked_reason: getConfiguredFieldValue(fields, 'JIRA_BLOCK_REASON_FIELD', ['customfield_11275']),
    blocked_action_taken: getConfiguredFieldValue(fields, 'JIRA_BLOCK_ACTION_FIELD', ['customfield_11377']),
    blocked_pending_with: getConfiguredFieldValue(fields, 'JIRA_BLOCK_PENDING_FIELD', ['customfield_11376', 'customfield_10101', 'customfield_10502'])
  };
}
...
export async function upsertIssuesToDatabase(rawIssues, options = {}) {
  const { onProgress, pruneObsolete = true } = options;
  ...
  if (pruneObsolete) {
    ...
    await onProgress?.('Sync filtrado: tickets fora do filtro foram preservados no Supabase.');
  }
...
export function buildDashboardData(rawIssues) {
  ...
  rawIssues.forEach(rawIssue => {
    ...
    const hiddenAssignee = isHiddenRjaUser({
      name: issue.assignee_name,
      email: issue.assignee_email,
    ...
    if (projectKey) {
      ...
      if (assigneeId && !hiddenAssignee) projectsMap[projectKey].analysts.add(assigneeId);
