...
const DOCWISE_PROJECT_KEY = 'DOCW';
const TRACKED_PROJECT_KEYS = Object.freeze([CRAWFORD_PROJECT_KEY, DOCWISE_PROJECT_KEY]);
const PAGE_SIZE = 100;
...
function collectIssueSignals(issue) {
  const fields = issue?.fields || {};
  const raw = issue?.raw_fields || {};
  const components = fields.components || issue?.components || raw.components || [];
  const labels = fields.labels || issue?.labels || raw.labels || [];
  const fixVersions = fields.fixVersions || issue?.fix_versions || raw.fixVersions || [];
  return [
    issue?.key,
    issue?.issue_key,
    fields.project?.key,
    ...
    issue?.parent_title,
    raw.parent?.fields?.summary,
    ...components.map(component => component?.name || component),
    ...labels,
    ...fixVersions.map(version => version?.name || version)
  ];
}

function hasDocwiseSignal(issue) {
  return collectIssueSignals(issue).some(value => normalizeText(value).includes('docwise'));
}

export function resolveTrackedWorklogProjectKey(issue) {
  const projectKey = String(issue?.fields?.project?.key || issue?.project_key || '').toUpperCase();
  const issueKey = String(issue?.key || issue?.issue_key || '').toUpperCase();
  if (projectKey === CRAWFORD_PROJECT_KEY || issueKey.startsWith(`${CRAWFORD_PROJECT_KEY}-`)) return CRAWFORD_PROJECT_KEY;
  if (projectKey === DOCWISE_PROJECT_KEY || issueKey.startsWith(`${DOCWISE_PROJECT_KEY}-`) || hasDocwiseSignal(issue)) {
    return DOCWISE_PROJECT_KEY;
  }
  return null;
}
...
export async function fetchProjectWorklogsFromJira(issues, credentials, projectKey) {
  ...
  const normalizedProjectKey = String(projectKey || '').toUpperCase();
  const projectIssues = (issues || [])
    .filter(issue => resolveTrackedWorklogProjectKey(issue) === normalizedProjectKey);
  const rows = await mapWithConcurrency(projectIssues, WORKLOG_CONCURRENCY, async issue => {
    const issueKey = issue.key ?? issue.issue_key;
    let worklogs = [];
    try {
      worklogs = await fetchIssueWorklogs(
        credentials.baseUrl,
        issueKey,
        credentials.email,
        credentials.token
    ...
    return worklogs.map(worklog => normalizeWorklog(worklog, issue, syncedAt, normalizedProjectKey));
  });
  return rows.flat();
}
...
export async function syncTrackedProjectWorklogs(rawIssues, credentials, options = {}) {
  ...
  const issues = (rawIssues || []).filter(issue => TRACKED_PROJECT_KEYS.includes(resolveTrackedWorklogProjectKey(issue)));
  if (issues.length === 0) {
  ...
  const grouped = await mapWithConcurrency(issues, WORKLOG_CONCURRENCY, async issue => {
    const projectKey = resolveTrackedWorklogProjectKey(issue);
  ...
  const projectKeys = [...new Set(issues.map(issue => resolveTrackedWorklogProjectKey(issue)).filter(Boolean))];
...
export function syncCrawfordWorklogs(rawIssues, credentials, options = {}) {
  const crawfordIssues = (rawIssues || []).filter(issue => resolveTrackedWorklogProjectKey(issue) === CRAWFORD_PROJECT_KEY);
