...
export async function deleteResourceAllocation({ id, actor }, client) {
  const database = db(client);
  if (!validUuid(id)) throw Object.assign(new Error('Alocação inválida para remoção.'), { status: 400 });

  const previous = checked(await database.from('resource_allocations').select(ALLOCATION_COLUMNS).eq('id', id).maybeSingle());
  if (!previous) throw Object.assign(new Error('Alocação não encontrada.'), { status: 404 });

  const result = await database.from('resource_allocations').delete().eq('id', id);
  if (result.error) throw Object.assign(new Error('Não foi possível remover a alocação.'), { status: 503, cause: result.error });
  ...
  return allocationFromRow(previous);
}

export async function deleteResourceProject({ id, actor }, client) {
  const database = db(client);
  if (!validUuid(id)) throw Object.assign(new Error('Projeto inválido para remoção.'), { status: 400 });

  const previous = checked(await database.from('resource_allocation_projects').select(PROJECT_COLUMNS).eq('id', id).maybeSingle());
  if (!previous) throw Object.assign(new Error('Projeto não encontrado.'), { status: 404 });

  const linkedAllocations = checked(await database.from('resource_allocations').select(ALLOCATION_COLUMNS).eq('project_id', id));
  const result = await database.from('resource_allocation_projects').delete().eq('id', id);
  if (result.error) throw Object.assign(new Error('Não foi possível remover o projeto.'), { status: 503, cause: result.error });

  checked(await database.from('resource_allocation_events').insert({
    action: 'project.deleted',
    project_id: null,
    actor,
    before: { ...previous, allocations: linkedAllocations },
    after: null,
  }).select('id').single());

  return {
    project: projectFromRow(previous),
    allocations: linkedAllocations.map(allocationFromRow),
  };
}
