...
import { sprintIds, timestamp } from '../src/data/sprint-review.js';

const sleep = ms => new Promise(resolve => setTimeout(resolve, ms));
export function mayBelongToReview(issue, field, sprintId) {
  const expected = String(sprintId);
  const values = [issue.fields?.[field], ...(issue.changelog?.histories || []).flatMap(h => (h.items || [])
    .filter(item => item.fieldId === field || item.field === 'Sprint').flatMap(item => [item.from, item.to]))];
  // Unknown membership needs inspection; a substring match would leak unrelated comments.
  return values.some(value => { const ids = sprintIds(value); return ids === null || ids.includes(expected); });
}
...
export async function createReviewJiraClient({ connection, fetchImpl = fetch, delay = sleep, timeoutMs = 240_000 } = {}) {
  ...
  async function collect(key, boardId, sprintId, profile, { allowActive = false, includePostClosure = false } = {}) {
    ...
    const collection = { strategy: 'project_history', projectIssueCount: 0, historicalCandidates: 0, commentIssues: 0 };
    let nextPageToken;
    ...
    for (let pageIndex = 0; pageIndex < 1000; pageIndex++) {
      ...
      for (const issue of page.issues) if (!seen.has(issue.id)) {
        seen.add(issue.id);
        collection.projectIssueCount++;
        if (!allowActive && !includePostClosure && timestamp(issue.fields?.created) > timestamp(sprint.completeDate)) continue;
        issues.push(issue);
      }
    ...
    await Promise.all(Array.from({ length: Math.min(4, issues.length) }, async () => {
      while (cursor < issues.length) {
        ...
        const participant = mayBelongToReview(issue, profile.sprintField, sprintId);
        if (participant) collection.historicalCandidates++;
        // Only fetch sensitive comment bodies for possible sprint participants.
        if (!allowActive && participant) {
          issue.comments = await pages(`/rest/api/3/issue/${encodeURIComponent(issue.id)}/comment`, 'comments');
          collection.commentIssues++;
        }
        else issue.comments = [];
      }
    }));
    collection.durationMs = Date.now() - started;
    return { projectKey: key, boardId, sprint, profile, issues, scopeComplete: true, collection, fetchedAt: new Date().toISOString(), jiraBaseUrl: base.origin };
  }
