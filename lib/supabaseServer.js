...
if (serviceKeyLooksInvalid) {
  console.error('[Supabase] ERRO: chave Supabase invalida. Use service_role ou sb_secret_ no backend.');
}
  ...
  if (!supabaseUrl || !supabaseServiceKey || serviceKeyLooksInvalid) {
    return {
      ...
      error: serviceKeyLooksInvalid
        ? 'Chave Supabase invalida. Defina uma chave valida em SUPABASE_SERVICE_ROLE_KEY ou SUPABASE_SECRET_KEY.'
        : 'Supabase nao configurado. Defina SUPABASE_URL e uma chave backend privilegiada.'
    };
  }
