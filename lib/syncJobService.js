...
import { resolveTrackedWorklogProjectKey, syncTrackedProjectWorklogs } from './jiraWorklogService.js';

const JOB_TABLE = 'jira_sync_jobs';
const ACTIVE_STATUSES = ['queued', 'running'];
const ACTIVE_JOB_STALE_MS = 6 * 60 * 1000;
const AUTO_SYNC_STALE_MS = 35 * 60 * 1000;
const DEFAULT_JQL = 'project is not EMPTY ORDER BY updated DESC';
const DEFAULT_AUTO_SYNC_JQL = 'updated >= -45m ORDER BY updated DESC';
const GLOBAL_WORKLOG_SYNC_LOOKBACK_DAYS = 45;
const ENV_JQL = process.env.JIRA_JQL?.trim() || '';
const AUTO_SYNC_JQL = process.env.AUTO_SYNC_JQL?.trim() || '';
...
function readAutoSyncCredentialsFromEnv() {
  const credentials = readCredentialsFromEnv();
  if (!credentials) return null;

  return {
    ...credentials,
    jql: AUTO_SYNC_JQL || DEFAULT_AUTO_SYNC_JQL
  };
}
...
function issueUpdatedAt(issue) {
  const value = issue?.fields?.updated || issue?.jira_updated_at || issue?.updated;
  const time = value ? new Date(value).getTime() : 0;
  return Number.isFinite(time) ? time : 0;
}

export function issuesEligibleForWorklogSync(rawIssues, { scoped = false, now = new Date() } = {}) {
  const issues = (rawIssues || []).filter(issue => resolveTrackedWorklogProjectKey(issue));
  if (scoped) return issues;

  const cutoff = now.getTime() - GLOBAL_WORKLOG_SYNC_LOOKBACK_DAYS * 24 * 60 * 60 * 1000;
  return issues.filter(issue => issueUpdatedAt(issue) >= cutoff);
}
...
async function finalizeTimedOutJob(job) {
  ...
  const message = 'Sincronizacao interrompida por timeout do backend. A proxima execucao automatica tentara novamente.';
...
async function getSupabaseLatestSuccessfulJob() {
  if (!isConfigured || !supabase) return null;
  ...
  if (error) {
    console.warn('[sync-job] Nao foi possivel buscar ultimo job bem-sucedido:', error.message);
    return null;
  }
...
export async function createSyncJob(input = {}, sessionId = null, options = {}) {
  ...
  const baseJob = {
    ...
    credentials,
    scope: syncScope,
    pruneObsolete: options.pruneObsolete ?? !syncScope,
...
export async function runSyncJob(jobId, credentials = null) {
  ...
  try {
    ...
    if (rawIssues.length === 0 && current.allowEmpty) {
      await appendLog(jobId, 'Nenhum ticket alterado na janela incremental; banco mantido sem alteracoes.');
      ...
      await appendLog(jobId, 'Sincronizacao incremental concluida sem alteracoes.');
    ...
    const worklogIssues = issuesEligibleForWorklogSync(rawIssues, { scoped: isScopedSync });
    await appendLog(
      jobId,
      isScopedSync
        ? 'Sincronizando apontamentos de horas do escopo solicitado.'
        : `Sincronizando apontamentos de horas de ${worklogIssues.length} tickets Crawford/Docwise atualizados nos ultimos ${GLOBAL_WORKLOG_SYNC_LOOKBACK_DAYS} dias.`
    );
    try {
      const worklogResult = await syncTrackedProjectWorklogs(worklogIssues, safeCredentials, {
        onProgress: (message) => appendLog(jobId, message)
      });
      await appendLog(
        jobId,
 ...
 */
export async function executeAutoSync(source = 'cron', { forceScheduleCheck = false, processQueued = false } = {}) {
  ...
  // 1. Processa jobs pendentes na fila apenas quando chamado explicitamente para
  // isso. O agendamento automatico de 30 minutos nao pode herdar uma carga
  // completa antiga e estourar o limite serverless novamente.
  if (processQueued) {
    try {
  ...
  const credentialsFromEnv = readAutoSyncCredentialsFromEnv();
  ...
  // 3. Cria novo job incremental usando credenciais de ambiente.
  // A sincronizacao automatica roda a cada 30 minutos e deve ficar abaixo do
  // limite de execucao serverless. Por isso ela importa apenas tickets
  // alterados recentemente e nunca remove tickets antigos do banco.
  const { job, credentials } = await createSyncJob(
    credentialsFromEnv,
    `auto-sync-${source}`,
    { pruneObsolete: false, allowEmpty: true }
  );
...
/**
 * Garante frescor operacional quando o agendador externo atrasar/pular.
 * Nao substitui o cron: funciona como rede de seguranca acionada por uso real
 * do app, sem criar job paralelo quando ja existe sync rodando.
 */
export async function ensureRecentAutoSync(source = 'self-healing', { maxAgeMs = AUTO_SYNC_STALE_MS } = {}) {
  let active = (await getSupabaseActiveJob()) || getMemoryActiveJob();
  if (isActiveJobTimedOut(active)) {
    await finalizeTimedOutJob(active);
    active = null;
  ...
  const latestSuccess = (await getSupabaseLatestSuccessfulJob()) || getMemoryLatestJob();
  const latestFinishedAt = getJobTimestamp(latestSuccess, ['finished_at', 'finishedAt', 'updated_at', 'updatedAt', 'created_at', 'createdAt']);
  if (latestFinishedAt && Date.now() - latestFinishedAt < maxAgeMs) {
    return {
      success: true,
      skipped: true,
      reason: 'Sincronizacao recente o suficiente.',
      lastSyncAgeMs: Date.now() - latestFinishedAt
    };
  }

  return executeAutoSync(source, { forceScheduleCheck: false, processQueued: false });
}
