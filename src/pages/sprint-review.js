...
import { renderSprintSlides, exportSprintSlides, sprintSlidePages } from '../utils/sprint-review-render.js';
...
export async function renderSprintReview() {
  ...
  function profileForm() {
    ...
    return `<details class="sr-panel" ${p.version ? '' : 'open'}><summary>Regras do projeto e board ${p.version ? '(perfil salvo)' : '(configuracao obrigatoria)'}</summary>
      <p>Os status abaixo exigem mapeamento explicito. As regras sao versionadas e preservadas em cada review.</p>
  ...
  function evidence(ids) {
    return state.review.evidence.filter(e => ids.includes(e.id)).map(e => `<div class="sr-evidence">${link(e.issueKey)} <span class="muted">${esc(e.type)} · ${esc(date(e.timestamp, state.review.profile.timezone))}</span><p>${esc(e.text)}</p>${e.author ? `<small>${esc(e.author)}</small>` : ''}</div>`).join('');
  }
  ...
  function bind() {
    ...
    bindTable();
    root.querySelectorAll('[data-statement]').forEach(input => input.addEventListener('input', () => { state.edits[input.dataset.statement] = input.value; }));
    ...
    on('sr-reset-text', 'click', () => { state.edits = {}; draw(); });
    on('sr-ai', 'click', () => run(async () => {
      if (Object.keys(state.edits).length && !window.confirm('Substituir os textos revisados por novas sugestoes da NVIDIA?')) return;
      Object.assign(state, await api('/synthesize', { sourceId: state.sourceId, choices: state.choices }, 'POST'));
      state.edits = {}; state.acceptedWarnings = [];
    }));
    ...
    on('sr-preview', 'click', () => { root.querySelector('#sr-preview-area').innerHTML = state.snapshot && state.renders?.length ? state.renders.map(r => `<img src="data:image/png;base64,${esc(r.png)}" alt="Arte aprovada, pagina ${r.page}" style="width:100%;height:auto">`).join('') : renderSprintSlides({ review: currentReview(), approved: Boolean(state.snapshot) }); });
    ...
    on('sr-saved', 'change', event => { if (event.target.value) run(async () => {
      ...
      const { renders } = await api(`/snapshots/${snapshot.id}/art`);
      state.renders = [...new Map([...renders].reverse().map(r => [r.page, r])).values()].sort((a, b) => a.page - b.page);
      if (state.renders.length !== sprintSlidePages(state.review).length || state.renders.some((r, i) => r.page !== i + 1)) {
        state.renders = [];
      }
    }); });
    on('sr-close-dialog', 'click', () => root.querySelector('#sr-dialog').close());
  }
  function currentReview() { return { ...state.review, goalAssessment: state.snapshot?.payload.goal || (state.goal?.confirmed ? state.goal : null), statements: state.review.statements.map(s => ({ ...s, text: state.edits[s.id] ?? s.text })) }; }
  ...
  async function save() {
    const input = { sourceId: state.sourceId, choices: state.choices, acceptedWarnings: state.acceptedWarnings, edits: state.edits, goal: state.goal };
