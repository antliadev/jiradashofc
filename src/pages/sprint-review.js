...
export async function renderSprintReview() {
  ...
  function draw() {
    ...
    const jobStatus = analysisRunning ? `<div class="sr-analysis-status" role="status" aria-live="polite"><span class="sr-spinner" aria-hidden="true"></span><div><strong>Análise da Sprint Review em andamento</strong><p>${esc(state.analysisJob?.message || 'Preparando a consulta e carregando o histórico da sprint...')}</p><span class="sr-progress" aria-hidden="true"><i></i></span><small>Você pode permanecer nesta tela. O resultado aparecerá automaticamente.</small></div></div>` : state.analysisJob ? `<div class="${state.analysisJob.status === 'failed' ? 'sr-error' : 'sr-analysis-complete'}" role="status"><strong>${state.analysisJob.status === 'completed' ? 'Análise concluída' : 'Falha na análise'}</strong><p>${esc(state.analysisJob.message || '')}</p></div>` : '';
